 ...
 * @notice Base test contract that sets up a complete local Uniswap V4 environment
 * This provides a testing foundation with:
 * 1. All core contracts deployed (PoolManager, FullRange, etc.)
 * 2. Test tokens (MockERC20s)
 * 3. Helper functions for common operations (create pools, add liquidity, etc.)
 * 4. Test accounts with pre-loaded balances
 */
contract LocalUniswapV4TestBase is Test {
    ...
    function setUp() public virtual {
        ...
        poolManager = new PoolManager(0); // 0% protocol fee
        
        // Step 2: Deploy Policy Manager
        policyManager = new PoolPolicyManager(governance);
        
        // Initialize with default policy
        DefaultPoolCreationPolicy defaultPolicy = new DefaultPoolCreationPolicy();
        policyManager.setPoolCreationPolicy(address(defaultPolicy));
        
        // Step 3: Deploy FullRange components
        
        // Deploy Dynamic Fee Manager
        dynamicFeeManager = new FullRangeDynamicFeeManager(governance);
        
        // Deploy Liquidity Manager
        liquidityManager = new FullRangeLiquidityManager(address(poolManager), governance);
        ...
        // Step 5: Deploy FullRange hook at the mined address
        ...
        // Get the pool ID for future reference
        poolId = poolKey.toId();
    ...
    /**
     * @notice Helper function to approve tokens for test accounts
     * @param account The account to approve tokens for
     */
    function approveTokens(address account) internal {
    ...
    /**
     * @notice Helper function to add full range liquidity to the pool
     * @param account The account providing liquidity
     * @param liquidity The amount of liquidity to add
     */
