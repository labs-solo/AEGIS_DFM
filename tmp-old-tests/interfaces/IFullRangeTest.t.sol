...
contract FullRangeMock is IFullRange {
    ...
    mapping(PoolId => mapping(address => uint256)) public _userShares;
    address private _positions = address(0x123);
    ...
    function deposit(DepositParams calldata params) external override returns (BalanceDelta delta) {
        require(initializedPools[params.poolId], "Pool not initialized");
        _userShares[params.poolId][params.to] += params.amount0Desired;
        return toBalanceDelta(int128(int256(params.amount0Desired)), int128(int256(params.amount1Desired)));
    }
    ...
    function withdraw(WithdrawParams calldata params) 
        ...
        require(initializedPools[params.poolId], "Pool not initialized");
        require(_userShares[params.poolId][msg.sender] >= params.sharesBurn, "Insufficient shares");
        
        _userShares[params.poolId][msg.sender] -= params.sharesBurn;
        
        amount0Out = params.sharesBurn;
        amount1Out = params.sharesBurn;
        
        delta = toBalanceDelta(- int128(int256(amount0Out)), - int128(int256(amount1Out)));
        return (delta, amount0Out, amount1Out);
    }

    function reinvestFees() external override {
